...
export default function CareersPage() {
  ...
  const uniqueDepartments = useMemo(() => departments.filter((d, i, a) => a.findIndex((x) => x.name === d.name) === i), [departments]);
    ...
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-sm font-medium mb-6">
              <span className="flex items-center justify-center w-5 h-5 rounded-full bg-green-400 text-green-900"><Zap className="w-3 h-3" /></span>
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight mb-4 leading-tight">
              Join Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-cyan-200">Healthcare</span> Team
            </h1>
            <p className="text-lg sm:text-xl text-blue-100/80 mb-10 max-w-2xl mx-auto leading-relaxed">
              Find your next career opportunity at our network of hospitals. We&apos;re always looking for talented professionals to deliver exceptional patient care.
            </p>
            <form onSubmit={handleHeroSearch} className="relative max-w-2xl mx-auto">
              <div className="flex items-center bg-white rounded-2xl shadow-2xl shadow-black/20 p-1.5">
                <div className="flex items-center gap-3 flex-1 px-4">
                  ...
                  <input type="text" placeholder="Search by title, department, or keyword..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full py-3 text-slate-900 placeholder-slate-400 text-sm sm:text-base bg-transparent outline-none" />
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mt-6">
              ...
              {["Cardiologist", "Nurse", "Radiology", "Pharmacist"].map((tag) => (
                <button key={tag} onClick={() => { setSearchQuery(tag); setHeroSearch(tag); }} className="px-3 py-1 text-sm bg-white/10 hover:bg-white/20 rounded-full border border-white/10 transition-colors">{tag}</button>
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="job-listings" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{heroSearch ? `Results for "${heroSearch}"` : "All Open Positions"}</h2>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={departmentFilter} onChange={(e) => setDepartmentFilter(e.target.value)} className="w-full appearance-none bg-slate-50 border border-slate-200 text-sm rounded-xl px-4 py-2.5 pr-10 text-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors">
                    ...
                    {uniqueDepartments.map((d) => <option key={d.id} value={d.name}>{d.name}</option>)}
                  </select>
